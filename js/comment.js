$(function(){
		...
		$('#contentBox ul li').find('#commentBtn').on('click',function(){
			if( $('#contentBox ul li').find('#textarea').val() == '' ){
				...
				if($.cookie('user')){
					...
					$.ajax({
						type:"post",
						url:"../php/add_video_comment.php",
						data:{
							user:$.cookie('user'),
							type:$('#contentBox ul li').find('#type').val(),
							videoid:$('#contentBox ul li').find('#videoid').val(),
							comments:comments
						},
						async:true
					});
		...
		$.ajax({
			type:"post",
			url:"../php/show_video_commentCount.php",
			data:{
				videoid:$('#videoid').val()
			},
		...
		$.ajax({
			type:"post",
			url:"../php/show_video_zanCount.php",
			data:{
				id:$('#videoid').val()
			},
			success:function(response){
		...
		$.ajax({
			type:"post",
			url:"../php/show_video_comment.php",
			data:{
				videoid:$('#videoid').val()
			},
			success:function(response){ 
		    					...
		    					'<div class="commentRight">'+
			    				    ...
			    				    '<div id="answerBox">'+
				    				    ...
				    				    '<form id="answerForm" >'+
							    			...
							    			'<input type="hidden" name="commenttype" id="commenttype" value="'+value.type+'" />'+
				...
				$(this).find('#answer_Btn').on('click',function(){
					...
					if( $(commentList[commentindex]).find('#answer_textarea').val() == '' ){
						...
						if($.cookie('user')){
							...
							$.ajax({
								type:"post",
								url:"../php/add_video_answer.php",
								data:{
									user:$.cookie('user'),
									answer_comment:answer_comment,
									commentid:$(commentList[commentindex]).find('#commentid').val(),
									type:$(commentList[commentindex]).find('#commenttype').val()
								},
								success:function(response){
									if(response == 1){
