...
$(function(){
			...
			$.ajax({
				...
				success:function(response){
					var json = $.parseJSON(response);
						...
						$('#contentUl').append('<input type="button" class="addmores" value="加载更多..." />');
					    ...
					    tiezilist.each(function(i){
							...
							$(this).find('#commentBtn').on('click',function(){
								if( $(tiezilist[index]).find('#textarea').val() == '' ){
									...
									if($.cookie('user')){
										var src = $(tiezilist[index]).find('#pinglunFace').attr('src');
							...
							$.ajax({
								...
								success:function(response){ 
									...
									//加载更多评论
									var id = $(tiezilist[index]).find('#articleid').val()
									$(tiezilist[index]).find('#comments').append('<a href="addmore.php?id='+id+'" class="addmoresPinglun" target="_blank">查看更多评论...</a>');
									
									if (6 > allcount ) {
										$(tiezilist[index]).find('#comments').find('.addmoresPinglun').hide();
									}
			...
			//															//console.log(placeholder)
								...
								async:true
														...
														var add1 = 0;
														if (page > tiezicount) {
															$('#contentUl').find('.addmores').off('click');
															$('#contentUl').find('.addmores').hide();
														}
														$('#contentUl').find('.addmores').on('click',function(){
															var scrollTop = $(document).scrollTop();
															$('#contentUl').find('.addmores').attr('disabled',true);
															$.ajax({
																type:"post",
																url:"php/show_content.php",
																data:{
																...
																beforeSend : function (jqXHR, settings) {
																	$('#contentUl').find('.addmores').val('');
																	$('#contentUl').find('.addmores').css('background','url(img/loading4.gif) no-repeat center')
																},
																success:function(response){
																	...
																	$('#contentUl').children('li').last().after(html_more);
																	...
																	$('#contentUl').find('.addmores').val('加载更多...');
																	$('#contentUl').find('.addmores').css('background','');
																	$('#contentUl').find('.addmores').attr('disabled',false);
																    ...
																    tiezilist.each(function(i){
																    	...
																    	//发表评论
																		$(this).find('#commentBtn').on('click',function(){
																			if( $(tiezilist[indexs]).find('#textarea').val() == '' ){
																				var tishi = $('#tishi');
																					tishi.show();
																					$('#tishi').find('p').html('评论内容不得为空哦！');
																					center(tishi,200,40);
																		...
																		$.ajax({
																			...
																			success:function(response){ 
																				...
																				//加载更多评论
																				var id = $(tiezilist[indexs]).find('#articleid').val()
																				$(tiezilist[indexs]).find('#comments').append('<a href="addmore.php?id='+id+'" class="addmoresPinglun" target="_blank">查看更多评论...</a>');
																				
																				if (6 > allcount ) {
																					$(tiezilist[indexs]).find('#comments').find('.addmoresPinglun').hide();
																				}
